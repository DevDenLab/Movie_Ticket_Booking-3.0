...
class Booking(mod.Model):
    ...
    seats = mod.ManyToManyField(CinemaHallSeat, blank=True, related_name="seatsviews")
    ...
    slug = AutoSlugField(
        populate_from=["user__username"], unique=True, editable=False, default=""
    )
            ...
            if old_instance.payment_status != self.payment_status:
                if self.payment_status == PaymentStatus.objects.get(id=3):
                    ...
                    for seat in ShowWiseSeats.objects.filter(
                        cinema_hall_seat_id__in=self.seats.all(),
                    ...
                    for seat in ShowWiseSeats.objects.filter(
                        ...
                        seat.state_id = SeatState.objects.get(id=2)
                        seat.save()
